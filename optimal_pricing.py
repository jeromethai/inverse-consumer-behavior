...
def solver(Q, r, i, d, p, soft=1e6, max_iter=3):
    ...
    i: computes optimal price for product i, 1<=i<=n
    d: target demand for product i
    p: full price vector (price of product i, p^i, is arbitrary) 
    max_iter: number of iterations
    """
    n = len(r)
    H = matrix(0., (1,n)); H[i-1] = 1.
    for k in range(max_iter):
        x = x_solver(Q, r, p, H, d, soft)
        p[i-1] = max((H*(Q*x+r))[0], 0)
    return p
